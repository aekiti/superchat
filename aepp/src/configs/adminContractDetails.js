export default {
  contractAddress: 'ct_2DJWtyBDKqeWb8sYm6fQLretRVtSVrETGT8JFYGgraWTPmPooc',
...
contract MessageContract =
  record message = { content : string, time : int, sender : address }
...
contract SuperChatAdmin =
  ...
  entrypoint get_superchat_profiles() : map(address, ProfileContract.user) =
    check_admin()
    state.profile.get_all_profile()

  entrypoint get_superchat_friends() : map(address, list(address)) =
    check_admin()
    state.friends.get_all_friends()

  entrypoint get_superchat_messages() : map(address, map(address, list(MessageContract.message))) =
    check_admin()
    state.messages.get_all_messages()

  entrypoint get_superchat_funds() : map(address, map(address, list(FundContract.fund))) =
    check_admin()
    state.funds.get_all_funds()

  entrypoint get_fund_contract_balance() : int = 
    check_admin()
